...
@Tag(name = "Owner Staff", description = "Manage staff members for owner accounts")
public class OwnerStaffController {

    private final StaffService staffService;

    public OwnerStaffController(StaffService staffService) {
    ...
    public ResponseEntity<List<StaffMemberDto>> listStaff(
        ...
        return ResponseEntity.ok(staffService.listStaffForOwner(userDetails.getUserId()));
    }
    ...
    public ResponseEntity<Void> removeStaff(
            @AuthenticationPrincipal CustomUserDetails userDetails,
            @PathVariable Long id) {
        staffService.removeStaffForOwner(userDetails.getUserId(), id);
