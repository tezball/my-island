...
import type { SupplierReview } from '../../types/review';
...
export const SupplierReviewsPage: React.FC = () => {
    ...
    const loadReviews = async () => {
        setIsLoading(true);
        try {
            const data = await reviewService.getMySupplierReviews();
            setReviews(data);
        } catch (error) {
    ...
    const handleRespond = async (reviewId: string) => {
        ...
        try {
            const updated = await reviewService.respondToSupplierReview(reviewId, responseText.trim());
    ...
    return (
        <div className="max-w-4xl">
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h1 className="text-2xl font-bold text-[#111418] dark:text-white">Customer Reviews</h1>
            ...
            {reviews.length === 0 ? (
                <div className="text-center py-16 bg-white dark:bg-[#1a2632] rounded-xl border border-gray-200 dark:border-gray-800">
                    ...
                    <p className="text-gray-500 text-sm">Reviews from your customers will appear here.</p>
                </div>
            ) : (
                <div className="space-y-4">
                    {reviews.map((review) => (
                        <div key={review.id} className="bg-white dark:bg-[#1a2632] p-5 rounded-xl border border-gray-200 dark:border-gray-800">
                            <div className="flex items-start justify-between mb-3">
                                <div>
                                    <div className="flex items-center gap-2">
                                        ...
                                        <p className="text-sm text-gray-500">{review.offerTitle}</p>
                                    </div>
                            ...
                            {review.supplierResponse ? (
                                <div className="pl-4 border-l-2 border-primary bg-gray-50 dark:bg-gray-800/50 p-3 rounded-r-lg mt-3">
                                    ...
                                    {review.supplierResponseAt && (
                                        <p className="text-xs text-gray-400 mt-1">Responded on {formatDate(review.supplierResponseAt)}</p>
                                    )}
                                </div>
                            ...
                            ) : review.moderationStatus === 'APPROVED' && !review.supplierResponse ? (
                                <button
