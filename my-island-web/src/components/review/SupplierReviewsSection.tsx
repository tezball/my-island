...
import type { SupplierReview, SupplierReviewEligibility } from '../../types/review';
...
export const SupplierReviewsSection: React.FC<SupplierReviewsSectionProps> = ({ supplierId, supplierRating, reviewCount }) => {
    const { user } = useAuth();
    const [reviews, setReviews] = useState<SupplierReview[]>([]);
    const [eligibility, setEligibility] = useState<SupplierReviewEligibility | null>(null);
    ...
    const loadEligibility = async () => {
        try {
            const data = await reviewService.checkSupplierEligibility(supplierId);
            setEligibility(data);
            const firstEligible = data.eligibleClaims.find(ec => !ec.alreadyReviewed);
            if (firstEligible) {
                setSelectedClaimId(firstEligible.claimId);
            }
        } catch {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!selectedClaimId || rating === 0 || comment.length < 10) return;

        setIsSubmitting(true);
        setSubmitError(null);

        try {
            await reviewService.createSupplierReview({
                offerClaimId: parseInt(selectedClaimId, 10),
                rating,
                comment,
            });
    ...
    const eligibleClaims = eligibility?.eligibleClaims.filter(ec => !ec.alreadyReviewed) ?? [];
    const allReviewed = eligibility?.eligibleClaims.length
        ? eligibility.eligibleClaims.every(ec => ec.alreadyReviewed)
        : false;

    return (
        <div className="mb-10">
            <h2 className="text-2xl font-bold text-[#111418] dark:text-white mb-6">Customer Reviews</h2>

            {/* Summary Bar */}
            {(supplierRating || reviews.length > 0) && (
                <div className="flex items-center gap-3 mb-6 bg-white dark:bg-[#1a2632] p-4 rounded-xl border border-gray-100 dark:border-gray-800">
                    <StarDisplay rating={supplierRating ?? 0} size="lg" />
            ...
            {/* Write Review Form */}
            {user && eligibleClaims.length > 0 && !submitSuccess && (
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        {eligibleClaims.length > 1 && (
                            <div>
                                ...
                                >
                                    {eligibleClaims.map((ec) => (
                                        <option key={ec.claimId} value={ec.claimId}>
                                            {ec.offerTitle} (redeemed {formatRelativeDate(ec.redeemedAt)})
                                        </option>
                                    ))}
                                </select>
                            </div>
            ...
            {user && allReviewed && !submitSuccess && (
                <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4 mb-6">
                    <p className="text-gray-600 dark:text-gray-400 flex items-center gap-2">
                        ...
                        You&apos;ve already reviewed all your redeemed offers from this supplier.
                    </p>
                </div>
            )}
                        ...
                        <div key={review.id} className="bg-white dark:bg-[#1a2632] p-5 rounded-xl border border-gray-100 dark:border-gray-800">
                            <div className="flex items-start justify-between mb-2">
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500">{review.offerTitle} · {formatRelativeDate(review.createdAt)}</p>
                                </div>
                            ...
                            {review.supplierResponse && (
                                <div className="mt-4 pl-4 border-l-2 border-primary">
                                    <p className="text-xs font-semibold text-primary mb-1">Supplier Response</p>
                                    <p className="text-gray-600 dark:text-gray-400 text-sm">{review.supplierResponse}</p>
                                    {review.supplierResponseAt && (
                                        <p className="text-xs text-gray-400 mt-1">
                                            Responded {formatRelativeDate(review.supplierResponseAt)}
                                        </p>
                                    )}
                                </div>
                            )}
