                ...
                <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <div className="size-8 rounded-lg bg-primary flex items-center justify-center text-white">
                            <span className="material-symbols-outlined text-xl">cabin</span>
                        </div>
                        <span className="text-lg font-bold text-[#111418] dark:text-white">Owner</span>
                    </div>
                    <button
                        onClick={() => setIsMobileMenuOpen(false)}
...
const OwnerNavLink: React.FC<{ to: string; icon: string; label: string; end?: boolean }> = ({ to, icon, label, end }) => {
    return (
        <NavLink
            to={to}
            end={end}
