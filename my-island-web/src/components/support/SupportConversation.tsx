...
interface SupportConversationProps {
    ...
    getMessages: (ticketId: string) => Promise<SupportTicketMessage[]>;
    sendMessage: (ticketId: string, content: string) => Promise<SupportTicketMessage>;
}
...
export const SupportConversation: React.FC<SupportConversationProps> = ({
    ...
    const handleSend = async () => {
        ...
        setError(null);
        try {
            const sent = await sendMessage(ticketId, newMessage.trim());
                                ...
                                <div className={`max-w-[80%] ${isOwn ? 'order-2' : 'order-1'}`}>
                                    {!isOwn && (
                                        <p className="text-xs text-gray-500 dark:text-gray-400 mb-1 px-1">
                                            {msg.senderName}
                                            {msg.isAdmin && (
                                                <span className="ml-1.5 inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300">
                                                    Admin
                                                </span>
                                            )}
                                        </p>
                                    ...
                                    <div
                                        className={`rounded-2xl px-4 py-2.5 ${
                                            isOwn
                                                ...
                                                : msg.isAdmin
                                                    ? 'bg-red-50 dark:bg-red-900/20 text-[#111418] dark:text-white rounded-bl-md border border-red-200 dark:border-red-800'
