...
| `postgres-catalog` | Postgres MCP → db `catalog` (`place` and Flyway tables) | **No** — same `ops_reader`. Postgres cannot `SELECT` across databases from one DSN |
...
`ops_reader` grants on db `catalog` are applied at Postgres init and again by `./scripts/dev up` (`ops/observability/postgres-grant-catalog-reader.sql`). SELECT only.
...
2. Add **stdio** (not HTTP) servers that match `.cursor/mcp.json`: `grafana` (`uvx mcp-grafana --disable-write`, `GRAFANA_URL=http://127.0.0.1:3030`, local admin/admin), `postgres` (DSN db `ops`), `postgres-catalog` (DSN db `catalog`). Stdio runs **inside the agent VM**, so loopback is the compose stack.
...
4. Enable the servers for Cloud Agents. Next run should list `grafana` / `postgres` / `postgres-catalog` tools.
