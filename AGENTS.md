...
Product canon (read-only until `PRD-*` + `implement`): `product/`. House stack (CEO 2026-09-05): **Java / Spring Boot**, **Vite + React PWA** (not Next.js), **PostgreSQL 17 + PostGIS**, Flyway, Grafana OSS MCP — `product/STACK.md`. Historical app: `docs/old/` and tag `legacy-platform` — not a migration source. Open **`ops/`** in Obsidian, not `docs/`. Do not recommend FastAPI, Neon, or Vercel as defaults.
