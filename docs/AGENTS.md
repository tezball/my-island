...
7. Chat agents do not merge PRs (Actions squash-merges ready PRs when CI is green **and** a valid non-author `APPROVED` exists). Do not invent a production deploy. Grafana and Postgres MCP are read-only.
...
Do not merge PRs from chat. Ready PRs squash-merge when CI is green **and** a valid non-author `APPROVED` exists. There is no production environment.
