...
Prefer `mcp-grafana` PromQL `up{job="catalog"}` → **1** while catalog is healthy. `--disable-write`. Postgres MCP: `SELECT` only on db `ops` (`mcp_ping`). Catalog `place` SELECT waits on **Engineering grants** (TODO on [[ops/tickets/WF-016]] / [[ops/workflow/MCP]]).

If grafana/postgres MCP tools are **missing** (Cloud Agent until a human adds dashboard **stdio** matching `.cursor/mcp.json`), HTTP:
...
# expect results.A.status 200; local Grafana is admin/admin ([[ops/workflow/LOCAL]])
```
