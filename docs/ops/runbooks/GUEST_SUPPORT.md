...
| Feature request | [[ops/agents/roles/product]] — `PRD-*` `inbox`, do not promise a date |
| Account / data / “delete my data” | [[ops/agents/roles/trust-safety]] — `INC-*` P0. Do not dump personal data into the vault |
| Abuse, scam, unsafe listing | Trust-safety. Stop public replies that argue the facts |
| “Site down” / payment | [[ops/agents/roles/ops-incidents]] — `INC-*`. There is no prod: say so, still log it |
| Host wants to list | [[ops/agents/roles/host-onboarding]] + [[LISTING_ROLLOUT]] |
...
4. Log a line in today’s [[ops/daily/_index|daily note]].
