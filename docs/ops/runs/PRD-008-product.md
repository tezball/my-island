...
- [[ops/tickets/PRD-008]] Notes: one Ireland geography model; map `country: NI` (and `IE`) onto the catalog’s 32 county rows; no Spring country table/enum. Verify checklist item for that mapping.
- [[ops/tickets/PRD-007]] Notes: Research county hints must fit the same 32-county list.
