...
- Planner writes `docs/ops/plans/<id>.md`, sets `status: plan`, opens a PR, **stops**.
- Human sets `status: implement` (or plan `status: approved`).
- Implementer branches, codes, opens a **different** PR, sets `status: review`. Does not merge.
