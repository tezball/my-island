...
Workshop: [[ops/workshops/e2e-place-stub]]. Canvas: [[e2e-place-stub]] (`ops/workflow/e2e-place-stub.canvas`).
...
- Run ≥2 assaults. Write each gap in `ops/runs/` or a ticket. **Automation** owns gap tickets ([[ops/agents/roles/automation-expert]]).
...
2. Optional: Postgres-RO MCP (`SELECT` only). `.cursor/mcp.json` targets db `ops`. Catalog `place` SELECT for `ops_reader` is **TODO Engineering** (grants + optional second DSN) — see [[ops/tickets/WF-016]] / [[ops/workflow/MCP]].
...
- [ ] Refs: [[ops/tickets/E2E-001]] · [[e2e-place-stub]] (`ops/workflow/e2e-place-stub.canvas`) · [`product/STACK.md`](../../product/STACK.md).
