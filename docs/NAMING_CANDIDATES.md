---
title: Naming Candidates
type: product
status: draft
created: 2026-09-01
tags:
  - branding
  - naming
---

# Naming Candidates

## Why rename

The current name and codebase encode a **camping/glamping** platform for Ireland:

- `PropertyType` = `TENT, TOURING, GLAMPING, CABIN, MOBILE_HOME`
- `Owner` *is* the campsite (one owner = one campsite)
- Guest-facing route is `/campsite/:id`, the entity is `Lot` (a pitch)

The rebooted product is broader: **any local entry** — campsite, B&B, guesthouse, cabin, local
supplier, experience/activity operator — plus editorial content (hikes, points of interest) and a
personal travel record (places you've been, on a map, shareable). The name should carry
*place + journey + local hospitality*, not *tent*.

## Trademark landmines — avoid these

Ireland's tourism brands are state-owned and heavily protected. Do **not** build on:

| Avoid | Owner |
|---|---|
| Fáilte / Fáilte Ireland | Fáilte Ireland (National Tourism Development Authority) |
| Wild Atlantic Way | Fáilte Ireland |
| Ireland's Ancient East / Hidden Heartlands | Fáilte Ireland |
| Discover Ireland | Fáilte Ireland |
| Tourism Ireland | Tourism Ireland |

Also crowded: anything `*-bnb`, `*-booking`, `Camp*`, `Pitch*`.

---

## Tier 1 — Recommended shortlist

| Name | Origin / meaning | Why it works | Risks |
|---|---|---|---|
| **Cairn** | A stack of stones marking a trail; walkers add a stone as they pass | Near-perfect fit. The core loop *is* "visit a place, leave your mark, build a record". One syllable, spellable, pronounceable by every tourist, works as a verb ("cairn it"). Scales past Ireland. | Generic-ish word; `cairn.com` taken, `.ie`/`cairn.travel` likely available. Existing Cairn Energy / Cairn Homes (different classes) |
| **Rian** | Irish: *trace, track, the mark left behind* (REE-un) | Names the journal/passport feature directly and beautifully. Short, soft, ownable. Genuinely Irish without being a cliché | Pronunciation needs a one-time cue. Also a given name |
| **Waymark** | The painted/posted markers along a waymarked trail | Bridges both halves: trail markers for hikers, "mark your way" for the travel record. Ireland has an official *National Waymarked Trails* network, so it reads native | Compound English; less distinctive than Cairn |
| **Inis** | Irish: *island* (IN-ish) — as in Inis Mór, Inis Oírr | Keeps the "my island" DNA, drops the camping baggage. Short, clean, unmistakably Irish, easy to say | Very common Irish place-name prefix, so SEO is contested |
| **Turas** | Irish: *journey, trip, pilgrimage* (TUR-us) | Literally the product noun. Covers stays, experiences and trails in one word | Slightly abstract; two syllables |
| **Meitheal** | Irish: the old custom of neighbours gathering to help with each other's work (MEH-hal) | Best *story* of the lot — it is exactly a two-sided local marketplace of hosts and suppliers helping each other. Superb brand narrative for the partner side | Hardest to pronounce/spell for international users. Consider as a partner-programme sub-brand rather than the main mark |

**Recommendation:** **Cairn** for the consumer brand — it survives internationalisation, is trivially
pronounceable, and the "leave a stone at each place" metaphor gives you a ready-made visual system,
map pin, loyalty mechanic and copy voice. Hold **Meitheal** as the name of the host/supplier
partner programme.

---

## Tier 2 — Strong alternatives

### Irish-rooted

| Name | Meaning |
|---|---|
| **Slí** / **Sli** | Way, route (as in *Slí na Sláinte*, the walking-route network) |
| **Bealach** | Path, pass, route |
| **Cuan** | Harbour, haven |
| **Bothán** | Hut, cabin, bothy |
| **Scéal** | Story, tale — fits reviews + journal |
| **Fánaí** | Wanderer, rover |
| **Áit** | Place. *Áitiúil* = local |
| **Ceilí** | A gathering |
| **Lóistín** | Lodging, accommodation |
| **Nead** | Nest |
| **Aisling** | Vision, dream |
| **Céim** | Step |
| **Dúiche** | District, native territory, "one's own country" |

### Island / geography

| Name | Note |
|---|---|
| **Isleway** | Island + route; compound but clear |
| **Inisway** | Irish *inis* + way |
| **Headland** | Coastal, solid, English |
| **Shoreline** | Warm, coastal |
| **Ninewaves** | From the mythic nine waves that bound Ireland's shore |
| **Emerald** | Obvious but crowded |

### Journey / map / record

| Name | Note |
|---|---|
| **Trailhead** | Where a journey begins; natural home for hikes/POIs |
| **Wayfare** | Old word for travel; *wayfarer* is the traveller |
| **Waypoint** | GPS-native; slightly technical |
| **Lodestar** | Guiding star |
| **Bearings** | "Get your bearings" |
| **Milestone** | Roadside stones; ties to the visit record |
| **Landmark** | Clear, but generic and contested |
| **Meander** | Slow travel; charming, low-urgency |
| **Roamside** | Coined; roam + roadside |
| **Pinned** | Map-pin native; very app-y |

### Hospitality

| Name | Note |
|---|---|
| **Hearth** | Warmth, welcome, staying |
| **Haven** | Safe harbour |
| **Homeground** | Local, rooted |
| **Perch** | Small, playful, a place to rest |
| **Nook** | Cosy, small-stay |

---

## Selection criteria — score candidates against these

1. **Scope-proof** — survives adding B&Bs, restaurants, experiences, and a second country
2. **Sayable** — an international visitor can pronounce it after seeing it once
3. **Spellable** — from hearing it spoken, without a fada/accent trap
4. **Ownable** — `.ie` + `.com` or a credible alt TLD; clear on EUIPO/IPOI classes 39/43
5. **Two-sided** — works in *both* "book your stay on X" and "list your business on X"
6. **Visual hook** — gives designers a mark, a map pin and a colour story for free

## Before committing

- [ ] Domain check: `.ie`, `.com`, `.travel`, `.co`
- [ ] Trademark search: IPOI (Ireland) and EUIPO, classes **39** (travel arrangement) and **43** (accommodation/food)
- [ ] Social handle availability across Instagram / TikTok / X
- [ ] Say it aloud on a phone call — does it survive being spelled over the phone?
- [ ] Check it does not mean something unfortunate in German, French, Spanish or Dutch (top inbound markets)

## Rename blast radius (for whichever name wins)

| Area | Change |
|---|---|
| Java package | `com.myisland.api` → `com.<name>.api` |
| Modules | `my-island-api`, `my-island-web`, `my-island-moderator` |
| Docker | image names, `docker-compose.yml` service names, network |
| Database | DB name, seed emails (`@myisland.com`), Flyway/migration scripts |
| Frontend | route `/campsite/:id` → `/listing/:id`, page names, copy, favicon, manifest |
| CI/CD | Jenkins jobs, `scripts/ci/*`, Grafana dashboard titles, Prometheus job labels |
| Docs | `docs/**` frontmatter and titles |
| GitHub | repo name (`tezball/my-island`), remote URL |

Do the rename **in the same pass as the `Owner` → `Partner`/`Listing` domain generalisation** —
both touch the same files, and doing them separately means paying the migration cost twice.
